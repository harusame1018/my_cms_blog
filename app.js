...
app.get("/news/:slug", (req, res) => {
    ...
    const news_file = fs.readFileSync(
        __dirname + "/posts/" + slug_param + ".md",
        "utf-8",
    );
...
app.post("/admin/post", upload.single("file"), (req, res) => {
    ...
    const filename =
        __dirname + "/posts/" + dt.toFormat("YYYY-MM-DD-HH24-MI") + ".md";
...
app.post("/admin/delete", (req, res) => {
    ...
    const delete_file_pass = path.join(
        __dirname,
        "posts",
        delete_post_name + ".md",
    );
...
app.post("/admin/edit/:id", (req, res) => {
    ...
    const file_content = gm.stringify(body, matter_data);
    fs.writeFile(
        path.join(__dirname, "posts", req.params.id + ".md"),
        file_content,
        "utf-8",
        (err) => {
            if (err) {
